...
const fetchFactory = (
  ...
  return fetchWithoutTimeout
    ? fetch(endpoint, {method: method, ...params}).then(processResponse)
    : fetchWithTimeOut(endpoint, {method: method, ...params}).then(
        processResponse,
      );
};
...
const makeHeader = type =>
  new Map([
    ...
    ['text', {headers: {Accept: 'text/plain', 'Content-Type': 'text/plain'}}],
  ]).get(type);

const makeRequest = (type, opts = {}) =>
  _.assignIn({...makeHeader(type), ...opts});
...
let _post = (endpoint, file, fetchWithoutTimeout, bypassAuth = false) => {
  const params = _addAuthIfRequired(
    makeRequest('json', {body: JSON.stringify(file)}),
    bypassAuth,
  );
  General.logDebug('Requests', `POST: ${endpoint}`);
  return params.then(headers =>
    fetchFactory(endpoint, 'POST', headers, fetchWithoutTimeout),
  );
};
