...
class ConventionalRestClient {
  ...
  postAllEntities(allEntities, onCompleteOfIndividualPost, onComplete) {
    ...
    const getResourceName = resourceName =>
      _.includes(ESResources, resourceName)
        ? `${resourceName}es`
        : `${resourceName}s`;
    const url = entity =>
  ...
  chainPostEntities(url, entities, onComplete) {
    return () => {
      ...
      entities.entities.map(entity => {
        return chainedRequest.push(
          chainedRequest.post(
            url,
            entity.resource,
            onComplete(entities.metaData, entity.resource.uuid),
          ),
        );
      });
  ...
  getAllForEntity(
    ...
    const searchFilter = !_.isEmpty(entityMetadata.resourceSearchFilterURL)
      ? `search/${entityMetadata.resourceSearchFilterURL}`
      : '';
    let settings = this.settingsService.getSettings();
    const resourceEndpoint = [
      settings.serverURL,
      _.isNil(entityMetadata.apiVersion) ? '' : entityMetadata.apiVersion,
      _.isNil(entityMetadata.resourceUrl)
        ? entityMetadata.resourceName
        : entityMetadata.resourceUrl,
      searchFilter,
    ]
  ...
  fireRequest(
    ...
    const processResponse = resp => {
      onGetOfAnEntity(
        entityMetadata,
        _.get(resp, `_embedded.${entityMetadata.resourceName}`, []),
      );
      afterGetOfEntity(entityMetadata.entityName, resp.page.totalPages);
    };

    const endpoint = (page = 0, size = settings.pageSize) =>
      `${resourceEndpoint}?${params(page, size)}`;
    return getJSON(endpoint()).then(response => {
      ...
      _.range(1, page.totalPages, 1).forEach(pageNumber =>
        chainedRequests.push(
          chainedRequests.get(endpoint(pageNumber), resp =>
  ...
  getAll(
    ...
    return _.reduce(
      ...
      (acc, entityMetadataWithSyncStatus) => {
        return acc.then(() =>
          this.getAllForEntity(
            entityMetadataWithSyncStatus,
            onGetOfAnEntity,
            onGetOfFirstPage,
            afterGetOfEntity,
            now,
