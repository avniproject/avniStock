...
class AuthService extends BaseService {
  ...
  authenticate(userId, password) {
    ...
    return Promise.resolve(settingsService.getSettings())
      ...
      .then(settings =>
        this._authIsStubbed(settings)
          ? {status: 'LOGIN_SUCCESS'}
          : authenticateAndUpdateUserSettings(
              userId.trim(),
              password,
              settings,
            ),
      ...
      return new Promise(resolve => {
        ...
        userPool.storage.sync(error => {
          if (error) {
            General.logDebug(
              'AuthService',
              'Could not sync memory storage from AsyncStorage. Ignoring. ',
            );
            resolve(null);
            return;
          }
  ...
  forgotPassword(userId) {
    return this._getSettings().then(settings => {
      return new Promise((resolve, reject) => {
        const cognitoUser = AuthService._createCognitoUser(
          settings,
          userId.trim(),
        );
        cognitoUser.forgotPassword({
          ...
          inputVerificationCode: function (data) {
            return resolve({
              status: 'INPUT_VERIFICATION_CODE',
              data: data,
              user: cognitoUser,
            });
          },
  ...
  changePassword(oldPassword, newPassword) {
    ...
    return this.getUser()
      .then(user =>
        this._authenticate(user.getUsername(), oldPassword, settings),
      )
      .then(result => {
  ...
  _authenticate(userId, password, settings) {
    const NEWPASSWORD_REQUIRED = 'NEWPASSWORD_REQUIRED',
