...
class ProgressbarStatus {
  ...
  onComplete(event, numberOfPages) {
    const entityMetaData = _.head(
      this.progressSteps.filter(step => step.name === event),
    );
    //SyncTelemetry is called twice during sync
    const syncWeight = !_.isNil(entityMetaData)
      ? entityMetaData.name === 'SyncTelemetry'
        ? entityMetaData.syncWeight / 2
        : entityMetaData.syncWeight
      : 0;
    this.progress +=
      syncWeight / ((numberOfPages === 0 ? 1 : numberOfPages) * 100);
  ...
  updateProgressSteps(entityMetadata, entitySyncStatus) {
    this.progressSteps = this.progressSteps.map(step => {
      const metadata = _.find(
        entityMetadata,
        metadata => metadata.entityName === step.name,
      );
      const entityTypeCount = entitySyncStatus.filtered(
        'entityName = $0',
        step.name,
      ).length;
