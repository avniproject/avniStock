...
class EntityQueueService extends BaseService {
  ...
  requeueAll() {
    ...
    const allTxEntityQueue = EntityMetaData.model()
      .filter(entityMetaData => entityMetaData.type === 'tx')
      .slice()
      .reverse()
      .map(entityModel =>
        this.findAll(entityModel.entityName).map(e =>
          EntityQueue.create(e, entityModel.entityName),
        ),
      )
      .reduce((acc, entityQueue) => acc.concat(entityQueue), []);
    this.bulkSaveOrUpdate(
      this.createEntities(EntityQueue.schema.name, allTxEntityQueue),
    );
  }
  ...
  getAllQueuedItems(entityMetadata) {
    const items = _.uniqBy(
      this.db
        .objects(EntityQueue.schema.name)
    ...
    const getEntity = ({entityUUID, entity}) =>
      this.findByKey('uuid', entityUUID, entity);
    return {
  ...
  getQueuedItemCount(entityName) {
    ...
    const entityItems =
      (entityName && allItems.filtered('entity = $0', entityName)) || allItems;
  ...
  popItem(uuid) {
    return () => {
      const itemToDelete = this.findByKey(
        'entityUUID',
        uuid,
